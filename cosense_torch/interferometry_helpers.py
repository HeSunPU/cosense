...
###############################################################################
def Obs_params_torch(obs, simim, snrcut=0.0, ttype='nfft', data_product='vis'):
	###############################################################################
	...
	if data_product == 'cphase_amp' or data_product == 'cphase_logcamp':
		...
		cphase_ind_list = None
		cphase_sign_list = None
	###############################################################################
...
###############################################################################
def eht_observation_pytorch(npix, nufft_ob, dft_mat, ktraj_vis, pulsefac_vis_torch, cphase_ind_list, cphase_sign_list, camp_ind_list, device, ttype='nfft', sigma=None, data_product='vis'):
	eps = 1e-16
	...
	def func(x):
		...
		if sigma is not None:
			vis_torch = vis_torch + torch.randn_like(vis_torch) * sigma
